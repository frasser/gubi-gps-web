...
L.Playback.PlayControl = L.Control.extend({
    ...
    onAdd : function (map) {
        this._container = L.DomUtil.create('div', 'leaflet-control-layers leaflet-control-layers-expanded');
        //this._container.setAttribute('class', 'opacity');
        var self = this;
        var playback = this.playback;
        playback.setSpeed(100);

        var playControl = L.DomUtil.create('div', 'playControl', this._container);
      //  playControl.setAttribute('class', 'opacity');
        this._button = L.DomUtil.create('button', '', playControl);
				this._button2 = L.DomUtil.create('button', '', playControl);
				this._button2.innerHTML= '<i class="fas fa-route"></i>';
        //this._button.innerHTML = 'Resproducir';
				//this._button.class= 'fa-play';
				this._button.innerHTML= '<i class="fa fa-play" ></i>';

				this._button2.setAttribute('class', 'button2');
				this._button.setAttribute('class', 'button2');


        this._velo = L.DomUtil.create('button', '', playControl);
				this._velo.innerHTML= '<i class="fas fa-tachometer-alt"></i>';
				this._velo.setAttribute('class', 'speed-menu ');
				this._velo.setAttribute('role', 'menu');
				this._velo.setAttribute('id', 'bot');

			/*	this._velo.onclick = function(){
        console.log('buttonClicked');
			}*/


				this._tx = L.DomUtil.create('span', '', playControl);
				this._tx.setAttribute('class', 'speed');
				this._tx.setAttribute('id', 'speed-icon-val');
				this._tx.setAttribute('value', '1');
				this._tx.innerHTML = '1';


				this._sli = L.DomUtil.create('div', '', playControl);
				this._sli.setAttribute('id', 'speed-slider');


				//this._velo.innerHTML = '<input id="speed-input" class="fa fa-play" type="text" value="1"><i class="fa fa-gear" ></i></input>';
			//	<input id="speed-input" class="span1 speed" type="text" value="1" />' +
				//this._velo.innerHTML = '<div id="speed-slider"></div>';
        ...
        function play(){
            if (playback.isPlaying()) {
                playback.stop();
                self._button.innerHTML = '<i class="fa fa-play"  ></i>';
								//self._button.removeClass('fa-play');
								//self._button.addClass('fa-pause');
            }
            else {
                playback.start();
                self._button.innerHTML = '<i class="fa fa-pause"  ></i>';
								//self._button.removeClass('fa-pause');
								//self._button.addClass('fa-play');
            }
        }
...
L.Playback.SliderControlVel = L.Control.extend({
    options : {
        position : 'bottomleft'
    ...
    onAdd : function (map) {
        this._container = L.DomUtil.create('div', 'leaflet-control-layers leaflet-control-layers-expanded');
        this._container.setAttribute('id','velocidad');
				this._container.innerHTML = 'jfjfjf';
        var self = this;
        var playback = this.playback;

        // slider
        this._sliderval = L.DomUtil.create('input', 'slider', this._container);
				this._sliderval.setAttribute('id','sli');
        this._sliderval.type = 'range';
        this._sliderval.min = -9;
        this._sliderval.max = 9;
				this._sliderval.step = .1;
				this._sliderval.value = 3;
       // this._slider.value =  self._speedToSliderVal(this.playback.getSpeed());
				this._sliderval.orientation = 'vertical';
        ...
        //.on(this._slider, 'dblclick', stop)
        .on(this._sliderval, 'click', L.DomEvent.preventDefault)
        //.on(this._slider, 'mousemove', L.DomEvent.preventDefault)
        .on(this._sliderval, 'change', onSliderChange, this)
        .on(this._sliderval, 'mousemove', onSliderChange, this);
        ...
        playback.addCallback(function (ms) {
            self._sliderval.value = ms;
        });


        map.on('playback:add_tracks', function() {
            self._sliderval.min = -9;
            self._sliderval.max = 9;
          //  self._slider.value = self._speedToSliderVal(this.playback.getSpeed());
        });
...
L.Playback = L.Playback.Clock.extend({
        statics : {
            ...
            SliderControl : L.Playback.SliderControl,
						SliderControlVel : L.Playback.SliderControlVel
        },
        ...
        initialize : function (map, geoJSON, callback, options) {
            ...
            if (this.options.dateControl) {
                ...
                this.dateControl.addTo(map);
						...
						if (this.options.sliderControlVel) {
								this.sliderControlVel = new L.Playback.SliderControlVel(this, options);
								this.sliderControlVel.addTo(map);
						}
        ...
        destroy: function() {
            ...
            if (this.dateControl) {
                this._map.removeControl(this.dateControl);
						...
						if (this.sliderControlVel) {
							this._map.removeControl(this.sliderControlVel);
						}
        }
    });
