...
$query = "SELECT nombre,fk_tipo_dispo,serie,latitud,longitud,velocidad,icono,fecha FROM datos,dispositivos WHERE serie=serie_dispositivo AND id_usuario_fk='".$idUsuario=$_SESSION['identificador']."' AND estado_dispo = 1 AND id IN (SELECT MAX(id) FROM datos GROUP BY serie)  ORDER BY id_dispositivo ASC" ;
/*$query = "SELECT nombre,fk_tipo_dispo,serie,latitud,longitud,velocidad,icono,max(fecha) FROM dispositivos,datos WHERE serie=serie_dispositivo AND id_usuario_fk='".$idUsuario=$_SESSION['identificador']."' AND estado_dispo = 1 group by serie  order by fecha desc " ;*/
